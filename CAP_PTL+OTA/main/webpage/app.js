...
function checkCredentials()
{
	...
	{
		$("#wifi_connect_credentials_errors").html("LOADING.....");
		connectWifi();    
	}
}
...
function get_list_SSID()
{ let text = "<select name='ssid_menu' id='ssid_menu_out' onchange =\"set_SSID_Value()\" ><option value=''>searched wifi SSIDs</option>";
	$.getJSON('/ssid.json', function(data) {
		//const myobj = JSON.parse(data);
	for(i in data.SSID){
		text += "<option value ="+data.SSID[i]+">"+data.SSID[i]+"</option>";
	}
	text+="</select>"
	document.getElementById("ssid_list").innerHTML = text;
	});
}
function set_SSID_Value(){
	var SSID_choosen = document.getElementById('ssid_menu_out');
	var SSID_value = SSID_choosen.options[SSID_choosen.selectedIndex].value;
	if(SSID_value != "");
	{document.getElementById('connect_ssid').value=SSID_value;}
	//$('#connect_ssid').val(SSID_value);
}
