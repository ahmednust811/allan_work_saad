...
static int g_loaded_creds_from_nvs = 0;
/**
...
void show_saved_JWT_KEY_START(void);
/**
	...
	if (event_base == WIFI_EVENT)
	{
		switch (event_id)
		{
			case WIFI_EVENT_AP_START:
				...
				ESP_LOGI(TAG,"starting AP at IP: %s",WIFI_AP_IP);
				break;
		...
		if (xQueueReceive(wifi_app_queue_handle, &msg, portMAX_DELAY))
		{
			switch (msg.msgID)
			{
				...
				case WIFI_APP_MSG_STA_DISCONNECTED:
					...
					break;
				case SAVED_BOOT_URL_TO_NVS:
					ESP_LOGI(TAG,"saved boot url to NVS");
					start_main_task();
					break;
				case JWT_ENCODING_SUCCESSFUL:
					ESP_LOGI(TAG,"JWT acquired");
					start_request_boot_url_task();
					break;
				case PARSING_DONE:
				{
					ESP_LOGI(TAG,"PARSING DONE, GOING FOR MQTT!!!");
					start_mqtt_gcp();
				}
				break;
				default:
