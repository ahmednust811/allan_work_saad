 ...
 */
char* createGCPJWT(const char* projectId, char* privateKey, size_t privateKeySize,uint8_t payload_type) {
...
void run(void *pvParameter) {
   ...
   esp_read_mac(&mac,ESP_MAC_WIFI_STA);
    char mac_adress[18];
    sprintf(mac_adress,"%02X%02X%02X%02X%02X%02X",mac[0],mac[1],mac[2],mac[3],mac[4],mac[5]);
    ...
    char* jwt = createGCPJWT(projectId,alzoid_key_pem, size,1);
    if(jwt != NULL){
    nvs_handle_t handle;
    esp_err_t err;
